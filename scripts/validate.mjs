...
check(["day", "week", "month", "year", "all", "custom"].every((period) => html.includes(`data-leaderboard-period="${period}"`)) && html.includes('id="leaderboard-rating-date"') && html.includes('id="leaderboard-custom-date-start"') && html.includes('id="leaderboard-custom-date-end"'), "leaderboard calendar period controls are incomplete");
check(["first", "previous", "next", "latest"].every((direction) => html.includes(`data-leaderboard-period-nav="${direction}"`)) && app.includes("navigateLeaderboardPeriod"), "leaderboard period navigation is incomplete");
check(/data-leaderboard-period="week" class="is-active" aria-pressed="true"/.test(html) && app.includes('leaderboardPeriod: "week"'), "Week must be the default leaderboard period");
...
check(["crystals", "score", "ratio", "time", "kills", "deaths", "golds", "kd"].every((icon) => html.includes(`data-metric-icon="${icon}"`)) && html.includes('class="metric-icon__slash">/</span>') && css.includes(".metric-icon__part--kills") && css.includes(".metric-icon__part--deaths") && css.includes('url("./icons/modes/deathmatch.svg")') && /\.metric-icon\[data-metric-icon="kd"\] \.metric-icon__slash\s*\{[\s\S]*?color:\s*var\(--color-ink\)/.test(css) && /\.metric-icon\[data-metric-icon="kd"\]\s*\{[\s\S]*?gap:\s*var\(--space-3xs\)/.test(css) && app.includes("metricIconMarkup") && miscIcons.every((icon) => icon.includes("<svg")), "shared metric artwork, spaced neutral K/D icon, or account-detail icon mapping is incomplete");
...
check(app.includes('leaderboardSort: "kills13"') && app.includes("leaderboardPeriodRange") && app.includes("calculatePeriodBetween(player"), "Kills / 13m default sorting or calendar leaderboard ranges are missing");
